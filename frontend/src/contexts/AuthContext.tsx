...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    let mounted = true;
    const raceWithAbort = <T,>(p: Promise<T>, signal: AbortSignal) => {
      return new Promise<T>((resolve, reject) => {
        const onAbort = () => reject(Object.assign(new Error('Aborted'), { name: 'AbortError' }));
        if (signal.aborted) return onAbort();
        signal.addEventListener('abort', onAbort);
        p.then((v) => { signal.removeEventListener('abort', onAbort); resolve(v); }).catch((e) => { signal.removeEventListener('abort', onAbort); reject(e); });
      });
    };
          ...
          if (userFromSession) {
            const enrichedUser: User = {
              ...
              user_metadata: userFromSession.user_metadata || (userFromSession as any).raw_user_meta_data || {},
                ...
                try {
                  // cancel previous profile request if any
                  if (profileRequestControllerRef.current) {
                    profileRequestControllerRef.current.abort();
                  }
                  profileRequestControllerRef.current = new AbortController();
                  const { data: profileData, error: profileError } = await raceWithAbort(
                    supabase
                      .from('profiles')
                      .select('role, is_active, full_name')
                      .eq('id', enrichedUser.id)
                      .single(),
                    profileRequestControllerRef.current.signal
                  );
                  ...
                  if (e && (e.name === 'AbortError' || e.message === 'Aborted')) {
                    // request was intentionally aborted (cleanup or new request started) - ignore
                  } else {
                    console.error('[AuthContext] profile lookup exception during init:', e?.message || e);
                  }
                }
          ...
          if (userData) {
            ...
            try {
              ...
              if (e && (e.name === 'AbortError' || e.message === 'Aborted')) {
                ...
                console.error('[AuthContext] profile lookup exception on auth change:', e?.message || e);
              }
            }
    ...
    return () => {
      mounted = false;
      try {
        // abort any pending profile request
        if (profileRequestControllerRef.current) {
          profileRequestControllerRef.current.abort();
        }
      } catch (e) {
        console.warn('[AuthContext] abort controller error during cleanup:', e);
      }

      try {
        // listener data may contain a subscription to unsubscribe
        const subscription = (listener as any)?.subscription || (listener as any);
  ...
  const logout = async () => {
    try {
      // abort any pending profile lookup first
      if (profileRequestControllerRef.current) {
        profileRequestControllerRef.current.abort();
      }
      await supabase.auth.signOut();
