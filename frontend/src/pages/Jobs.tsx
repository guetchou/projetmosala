    ...
    <div className="min-h-screen flex flex-col bg-[#f6f9fc]">
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-6xl">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {isLoading && (
            <div className="col-span-full text-center text-[#22304a]/70">Chargement des emplois...</div>
          )}
          {(jobsData || []).map((entity, idx) => {
            const att = (entity as any).attributes;
            const mediaUrl = (url?: string) => {
              if (!url) return "/topcenter-uploads/logo-mosala1.png";
              if (url.startsWith("http")) return url;
              const base = import.meta.env.VITE_STRAPI_URL || "http://localhost:1337";
              return `${base}${url}`;
            };
            const logo = att?.logo?.data?.attributes?.url as string | undefined;
            const title = att?.titre as string;
            const company = (att?.entreprise as string) || "";
            const location = (att?.lieu as string) || "";
            const description = (att?.description as string) || "";
            const salary = (att?.salaire as string) || "";
            return (
              <motion.div
                key={entity.id}
              ...
              >
                <div className="flex items-center gap-4 p-6 pb-0">
                  <img src={mediaUrl(logo)} alt="Logo entreprise" className="h-14 w-14 rounded-full border-2 border-[#ff7844]/20 shadow bg-white object-contain" />
                  <div>
                    <h3 className="text-lg font-bold text-[#22304a] mb-1">{title}</h3>
                    <div className="text-sm text-[#22304a]/70">{company}</div>
                  </div>
                </div>
                <div className="flex items-center gap-2 px-6 mb-2 text-xs text-[#ff7844]/70">
                  <MapPin className="h-4 w-4" /> {location}
                </div>
                <p className="text-[#22304a]/80 px-6 mb-4 text-sm line-clamp-3">{description}</p>
                <div className="flex justify-between items-center w-full mt-auto pt-4 px-6 border-t border-[#ff7844]/10">
                  <span className="font-bold text-[#22304a] text-lg">{salary}</span>
